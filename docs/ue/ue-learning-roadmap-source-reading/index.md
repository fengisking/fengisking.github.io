...
![UE 源码阅读闭环](./image/source-reading-loop.svg)

## 0. 这篇文章怎么读

这不是一份“把 UE 所有模块列出来”的路线图，而是一套源码阅读方法。目标是让你每次遇到项目问题时，都能把现象拆成：入口函数、调用链、关键变量、状态变化、验证方式。

优秀的 UE 源码阅读应该形成固定产物：

```text
问题现象：玩家移动抖动 / AI 不重新感知 / 软引用没进包
源码入口：从哪个类哪个函数开始断
调用链：从入口到最终状态改变的完整路径
关键变量：哪些变量决定分支
验证实验：怎么改一个条件证明理解正确
项目结论：项目里应该怎么用，哪些做法要避免
```

建议阅读顺序：

```text
对象系统：UObject / UClass / UPROPERTY / GC
运行时：World / Actor / Component / Tick
Gameplay：Input / Movement / Collision / AI
网络：Replication / RPC / Prediction / Correction
表现：Animation / Rendering / Physics
工程：Cook / Package / Insights / World Partition / Engine Upgrade
```
...
读完一篇源码文章后，不要只问“我看懂文字了吗”，而要做五个验证：

```text
1. 能不能说出入口类和入口函数。
2. 能不能画出从入口到结果的调用链。
3. 能不能列出 3 个决定分支的关键变量。
4. 能不能在源码里打断点复现这条链路。
5. 能不能改一个最小变量，观察结果按预期变化。
```

如果做不到第 4 点和第 5 点，只能算理解了概念，还没有真正掌握源码。每篇文章都应该尽量形成“断点 + 变量 + 现象”的闭环。
